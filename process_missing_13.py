...
# 讀取缺失檔案清單
with open('truly_missing_final.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
    missing_files = [item['sam_file'] for item in data['truly_missing']]

print(f"="*70)
print(f"批次整理13個真正缺失的原始稿")
print(f"="*70)
print(f"總數: {len(missing_files)} 個")
print()
...
def main():
    ...
    for i, filename in enumerate(missing_files, 1):
        ...
        if not os.path.exists(file_path):
            print(f"[{i:3d}/{len(missing_files)}] ✗ 檔案不存在: {filename}")
            errors.append((filename, '檔案不存在'))
            continue

        print(f"[{i:3d}/{len(missing_files)}] 處理中: {filename[:50]}...")

        status, result = process_transcript(file_path)

        if status == 'success':
            processed += 1
            successful_files.append((filename, result))
    ...
    print("處理完成")
    print("="*70)
    print(f"成功: {processed}/{len(missing_files)}")
    ...
    result_data = {
        ...
        'successful_files': [
            ...
            for src, out in successful_files
        ],
        'errors': [
            {'file': f, 'error': e}
            for f, e in errors
    ...
    with open('process_missing_13_result.json', 'w', encoding='utf-8') as f:
        json.dump(result_data, f, ensure_ascii=False, indent=2)

    print(f"處理結果已儲存至: process_missing_13_result.json")
