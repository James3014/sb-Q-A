...
| 功能 | 說明 |
|------|------|
| 🔗 推廣追蹤 | 完整點擊→試用→轉換→分潤鏈路追蹤 |
| 🛡️ 安全防護 | 多重驗證防止重複試用和濫用 |
| 📊 成效分析 | 智能分析推廣效果並提供優化建議 |
| 💰 自動分潤 | 按季度自動計算和結算分潤 |
| 🎯 精準歸因 | 準確追蹤每筆收益的推廣來源 |
...
| 功能 | 說明 |
|------|------|
| 📊 統計概覽 | 總合作方數、啟用數、總點擊數、總試用數、總分潤 |
| ➕ 新增合作方 | 建立合作方帳號、設定折扣碼和分潤率 |
| 👥 展開用戶列表 | 點擊 ▶ 查看該合作方帶來的所有用戶（含註冊時間） |
| 📈 即時統計 | 點擊數、試用數、轉換數、轉換率、總分潤 |
| 🔄 狀態管理 | 啟用/停用合作方帳號 |
| 🔗 推廣連結 | 自動生成專屬推廣連結（含試用參數） |
...
#### 推廣成效分析 (/admin/analytics)

| 功能 | 說明 |
|------|------|
| 📈 KPI 總覽 | 點擊/試用/轉換/分潤四大核心指標 |
| 🔄 轉換漏斗 | 視覺化點擊→試用→付費完整流程 |
| 🏆 合作方排行 | TOP 5 合作方按轉換數排序 |
| 💡 智能洞察 | 自動分析數據並提供優化建議 |
| ⏰ 時間切換 | 支援 7天/30天 數據分析 |
...
| 防護類型 | 機制 | 說明 |
|---------|------|------|
| 🔒 重複試用防護 | trial_used 狀態檢查 | 每用戶僅能試用一次 |
| 🌐 IP 濫用防護 | 單 IP 7天限5次 | 防止批量刷試用 |
| 📧 Email 濫用防護 | 同域名限3次 | 防止 +1 變體濫用 |
| ⏰ 帳號年齡檢查 | 新帳號等5分鐘 | 防止批量註冊 |
| 🎯 頁面保護 | 狀態驗證 | 防止直接訪問成功頁 |

#### 完整追蹤鏈路

```
用戶點擊推廣連結 → 記錄點擊 (affiliate_clicks)
    ↓
顯示推廣橫幅 → 說明來源和優惠
    ↓
用戶登入/註冊 → 安全檢查通過
    ↓
開始試用 → 記錄試用來源 (users.trial_coupon_code)
    ↓
用戶付費 → 自動計算分潤 (affiliate_commissions)
    ↓
季度結算 → 批次支付給合作方
```
...
│
├── app/api/admin/          # 🔒 Admin Server API（2025-11-29 新增）
│   ├── dashboard/          # Dashboard 數據 API
│   ├── users/              # 用戶管理 API
│   ├── lessons/            # 課程分析 API
│   ├── monetization/       # 付費分析 API
...
│   │   ├── create/         # 建立合作方
│   │   └── users/          # 合作方用戶列表
│   ├── commissions/        # 💰 分潤管理 API（2025-12-12 新增）
│   └── analytics/          # 📊 推廣分析 API（2025-12-12 新增）
│
docs/
...
- [x] 推廣統計與趨勢圖表
- [x] 季結算管理與批次支付
- [x] 推廣成效分析（轉換漏斗/排行榜/智能洞察）
- [x] 安全防護機制（防重複試用/IP限制/Email濫用防護）
