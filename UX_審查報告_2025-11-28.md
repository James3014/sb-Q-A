...
| 5. 記錄練習 | 點「完成練習」並評分 | 按鈕在頁面中段易漏掉、評分需跳轉彈窗 | 底部固定「完成練習」按鈕 + 嵌入式滑動評分 | 練習完成率 ↑35% | LessonDetail.tsx:56 |
...
4. **收藏與練習記錄分離**：「❤️ 收藏」與「📝 練習中心」是兩個獨立入口，難以關聯
...
- 收藏與練習記錄分離，使用者想回看「收藏的課程中，哪些我還沒練習」需在兩個頁面間切換
...
**練習中心頂部 Tab**：
```
[📊 改善度] [❤️ 收藏課程] [⚠️ 待加強] [📅 全部記錄]
```

- **改善度**：當前實作（保持）
- **收藏課程**：整合 favorites 表，顯示「已收藏但未練習」的課程（CTA：立即練習）
- **待加強**：篩選 `rating < 3` 的記錄，推薦重練
- **全部記錄**：當前「紀錄」Tab
...
| 4 | 練習記錄流程中斷 | 完成按鈕需跳轉彈窗評分 | 完成轉化率僅 65% | 嵌入式滑動評分卡（無跳轉） | 70% | 3 | 80% | 1週 | **168** | P0 |
...
| **單任務成功率** | 完成任務且無協助 | ≥80% | 觀察員記錄（成功/失敗/部分成功） |
| **路徑步數** | 從起點到完成的點擊/滑動次數 | 任務 1 ≤5 步、任務 2 ≤3 步 | 螢幕錄影 + 人工計數 |
| **誤觸率** | 點擊非預期目標的比例 | ≤10% | 點擊熱圖（Hotjar）或人工記錄 |
